...
describe('production public-demo build configuration', () => {
  it('keeps App Check site-key wiring optional and disables server enforcement', () => {
    ...
    const validationBlock = cloudbuild.slice(cloudbuild.indexOf('for value in'), cloudbuild.indexOf('done', cloudbuild.indexOf('for value in')));
    expect(validationBlock).not.toContain('_NEXT_PUBLIC_FIREBASE_APPCHECK_SITE_KEY');
